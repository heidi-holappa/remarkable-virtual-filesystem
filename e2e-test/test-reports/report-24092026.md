...
* [x] TC2: rename a directory
  0. start at root directory 
  1. input `rename test test-a`
  2. confirm path `test` is now `test-a`
  3. reset:
     4. input `rename test-a test`"
...
* [x] TC5: move file to itself is a no-op
  1. input: `mv /test/document-0.pdf /test/`
  2. expected: no-op
