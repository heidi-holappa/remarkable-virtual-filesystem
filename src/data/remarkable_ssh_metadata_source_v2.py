...
class RemarkableSSHMetadataSourceV2(MetadataSourceV2):
    ...
    def load(self) -> Dict[str, Entry]:
        ...
        data: Dict[str, Entry] = self._fetch_metadata()
    ...
    def _fetch_metadata(self) -> Dict[str, Entry]:
        """
        Manages the logic of populating data with data entries. Loaded
        in-memory data contains the metadata and combined size information
        Content-information is not needed for in-memory data operations so
        it is not fetched during load.
        """
        ...
        with tarfile.open(fileobj=BytesIO(tar_bytes), mode="r:") as tar:
            for member in tar:
                ...
                try:
                    ...
                    entry_metadata = Metadata.from_dict(metadata)
                    data_entry = Entry(metadata=entry_metadata, content=None, size=0)

                    data[entry_uuid] = data_entry
                except InvalidMetadataError as e:
                    print(f"ERROR: could not read {metadata["visibleName"]}. Cause: {e}")
