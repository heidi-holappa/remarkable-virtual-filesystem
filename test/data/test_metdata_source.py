...
class TestMetadataSource(unittest.TestCase):
    ...
    def test_load_raises_not_impelmented_error(self) -> None:
        with self.assertRaises(NotImplementedError) as ctx:
            self.source.load()


    def test_refresh_raises_not_impelmented_error(self) -> None:
        with self.assertRaises(NotImplementedError) as ctx:
            self.source.refresh()


    def test_write_raises_not_impelmented_error(self) -> None:
        with self.assertRaises(NotImplementedError) as ctx:
            self.source.write(
                UUID_FAIRYTALE,
