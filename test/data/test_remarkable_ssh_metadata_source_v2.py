...
The tests for _populate_data and _get_files_sizes were initially
...
class TestRemarkableSSHMetadataSourceV2(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.source = RemarkableSSHMetadataSourceV2()

    # --------------------------------------------------
    # Helpers
    # --------------------------------------------------

    def _create_tar_bytes(self, files: Dict[str, Entry]) -> bytes:
        """
        ...
        with tarfile.open(fileobj=tar_buffer, mode="w:") as tar:
            for uuid, entry in files.items():
                file_data = json.dumps(entry.metadata.to_dict()).encode("utf-8")
    ...
    @patch.object(RemarkableSSHMetadataSourceV2, "_get_file_sizes")
    @patch.object(RemarkableSSHMetadataSourceV2, "_fetch_metadata")
    def test_load_merges_sizes(self, mock_fetch: MagicMock, mock_sizes: MagicMock) -> None:
        ...
        metadata2 = Metadata(
            ...
            type=EntityType.DOCUMENT_TYPE,
            visible_name="doc2")
        entry1 = Entry(metadata=metadata1)
        entry2 = Entry(metadata=metadata2)
    ...
    def test_fetch_metadata_success(self, mock_popen: MagicMock) -> None:
        metadata1 = Metadata(
            created_time=0,
            last_modified=0,
            new=False,
        ...
        self.assertEqual(result["uuid1"].metadata.visible_name, "doc1")

        self.assertIn("uuid2", result)
        self.assertEqual(result["uuid2"].metadata.visible_name, "doc2")

    @patch("subprocess.Popen")
    @patch("src.data.remarkable_ssh_metadata_source_v2.Metadata.from_dict")
    @patch("builtins.print")
    def test_fetch_metadata_invalid_metadata(
            self,
        ...
        mock_from_dict.side_effect = InvalidMetadataError("invalid metadata")

        result = self.source._fetch_metadata()

        self.assertNotIn("uuid1", result)

        mock_print.assert_called_once_with(
            "ERROR: could not read bad-doc. Cause: invalid metadata"
        )

    @patch("subprocess.Popen")
    def test_populate_data_nonzero_returncode_raises(self, mock_popen: MagicMock) -> None:
        mock_proc = MagicMock()
    ...
    @patch("subprocess.Popen")
    def test_populate_data_no_tar_bytes_raises(self, mock_popen: MagicMock) -> None:
        mock_proc = MagicMock()
        # Make context manager return the same object
        mock_proc.__enter__.return_value = mock_proc
    ...
    @patch("subprocess.Popen")
    @patch.object(RemarkableSSHMetadataSourceV2, "_get_file_sizes")
    def test_populate_data_invalid_json_skipped(self, mock_sizes: MagicMock, mock_popen: MagicMock) -> None:
    ...
    def test_populate_data_skips_member_without_file(self) -> None:
        member = MagicMock()
    ...
    def test_restart_xochitl_success(self, mock_run: MagicMock) -> None:
        ...
        # ---- Call method ----
        RemarkableSSHMetadataSourceV2.restart_xochitl()
