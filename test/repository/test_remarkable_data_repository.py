...
from test.test_data_v2 import (
    ...
    UUID_INVALID_LAST_MODIFIED, UUID_A0_UNDER_B, UUID_D_1, UUID_FAIRYTALE_COPY)
...
class RemarkableWorkspaceTest(unittest.TestCase):
    ...
    def test_get_collection_when_current_collection_is_root_and_collection_is_not_found(self) -> None:
        assert self.repository.get_collection("C", "") == None
    ...
    def test_change_collection_from_root_to_direct_subpath(self) -> None:
        ...
        self.repository.change_collection("/A")
        assert self.repository.get_current_collection() == UUID_A
    ...
    def test_change_collection_from_root_to_direct_subpath_with_relative_directories(self) -> None:
        self.repository.change_collection("/././../A/..////A")
        assert self.repository.get_current_collection() == UUID_A

    def test_change_collection_to_nested_subpath_with_absolute_path(self) -> None:
        self.repository.change_collection("/A/A_0")
        assert self.repository.get_current_collection() == UUID_A0
    ...
    def test_change_collection_to_nested_subpath_with_relative_path(self) -> None:
        ...
        self.repository.change_collection("/A")
        self.repository.change_collection("../B/B_0")
        # then the new path should be /B/B_0
        assert self.repository.get_current_collection() == UUID_B0
    ...
    def test_change_collection_with_invalid_path_raises_error(self) -> None:
        with self.assertRaises(NoSuchFileOrDirectoryError) as context:
            self.repository.change_collection("C")
    ...
    def test_when_entry_is_not_document_or_collection_message_is_shown(self) -> None:
        # TODO: write test
        original_data = self.repository.get_data()
        invalid_data = {
            "some-uuid": {
                "type": "UnknownType",
                "parent": ""
        ...
        self.repository._data = invalid_data
        self.repository.process_ls([])
        ...
        self.repository._data = original_data
    ...
    def test_after_successful_move_without_path_in_filename_parent_is_updated(self, mock_write: MagicMock) -> None:
        ...
        self.repository.set_current_collection(UUID_A)
        self.repository.process_move_command("Fairytale-2.pdf", "/B")
        self.assertEqual(UUID_B, self.repository.get_data()[UUID_FAIRYTALE_2]['parent'])

    @patch.object(RemarkableSSHMetadataSourceV2, "write_metadata")
    def test_after_successful_move_with_absolute_path_in_filename_parent_is_updated(self,
                                                                                    mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_A)
        self.repository.process_move_command("/A/Fairytale-2.pdf", "/B")
        self.assertEqual(UUID_B, self.repository.get_data()[UUID_FAIRYTALE_2]['parent'])

    @patch.object(RemarkableSSHMetadataSourceV2, "write_metadata")
    def test_after_successful_move_with_relative_path_in_filename_parent_is_updated(self,
                                                                                    mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_A0)
        self.repository.process_move_command("../Fairytale-2.pdf", "/B")
        self.assertEqual(UUID_B, self.repository.get_data()[UUID_FAIRYTALE_2]['parent'])
    ...
    def test_invalid_source_filename_results_in_error_shown_to_user(self, mock_write: MagicMock) -> None:
        ...
        with patch('sys.stdout', new=StringIO()) as mock_out:
            ...
            self.repository.process_move_command(source, "/B")
            output: str = mock_out.getvalue()
    ...
    def test_invalid_source_path_results_in_error_shown_to_user(self, mock_write: MagicMock) -> None:
        ...
        self.repository.set_current_collection(UUID_A)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("/C", "/B")
            output: str = mock_out.getvalue()
            self.assertTrue(f"mv: cannot access /C: {NO_SUCH_FILE_OR_DIRECTORY} " in output,
                            msg=f"Output was: {output}")
    ...
    def test_with_invalid_target_path_error_is_shown_to_user(self, mock_write: MagicMock) -> None:
        ...
        self.repository.set_current_collection(UUID_A)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("/A/Fairytale.pdf", "/C")
            output: str = mock_out.getvalue()
    ...
    def test_document_with_invalid_metadata_cannot_be_moved_and_error_is_shown(self, mock_write: MagicMock) -> None:
        ...
        self.repository.set_current_collection(UUID_A)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("/A/InvalidLastModified.pdf", "/B")
            output: str = mock_out.getvalue()
    ...
    def test_collection_type_cannot_be_moved_to_its_descendant_with_relative_target_path(self) -> None:
        self.repository.set_current_collection(UUID_A)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("/A", "A_0")
            output: str = mock_out.getvalue()
            self.assertTrue("collection can not be moved into itself or its descendant" in output,
                            msg=f"Output was: {output}")

    # Constraint: A collection can not be moved into itself or its descendant
    def test_collection_type_cannot_be_moved_to_its_descendant_with_absolute_target_path(self) -> None:
        self.repository.set_current_collection(UUID_ROOT)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("/A", "/A/A_0")
            output: str = mock_out.getvalue()
            self.assertTrue("collection can not be moved into itself or its descendant" in output,
                            msg=f"Output was: {output}")
    ...
    def test_collection_cannot_be_moved_into_itself(self) -> None:
        self.repository.set_current_collection(UUID_ROOT)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("/A", "/A")
            output: str = mock_out.getvalue()
            self.assertTrue("collection can not be moved into itself or its descendant" in output,
                            msg=f"Output was: {output}")
    ...
    def test_root_collection_cannot_be_moved(self) -> None:
        self.repository.set_current_collection(UUID_ROOT)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("", "")
            output: str = mock_out.getvalue()
    ...
    def test_destination_cannot_contain_child_with_the_same_visible_name(self) -> None:
        self.repository.set_current_collection(UUID_ROOT)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("/A", "/B")
            output: str = mock_out.getvalue()
            self.assertTrue("destination must not contain a child with the same name" in output,
                            msg=f"Output was: {output}")
    ...
    def test_moving_to_the_same_parent_should_result_in_no_op(self, mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_A)
        self.repository.process_move_command("Fairytale.pdf", "/A")
    ...
    def test_move_with_wild_card_single_document(self, mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_A)
        self.repository.process_move_command("*le.pdf", "/B/B_0")
        self.assertEqual(mock_write.call_count, 1)
        self.assertEqual(UUID_B0, self.repository.get_data()[UUID_FAIRYTALE]['parent'])

    @patch.object(RemarkableSSHMetadataSourceV2, "write_metadata")
    def test_move_with_wild_card_single_document_absolute_path(self, mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_ROOT)
        self.repository.process_move_command("/A/*le.pdf", "/B/B_0")
        self.assertEqual(mock_write.call_count, 1)
        self.assertEqual(UUID_B0, self.repository.get_data()[UUID_FAIRYTALE]['parent'])
    ...
    def test_move_with_wild_card_single_document_relative_path(self, mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_B)
        self.repository.process_move_command("../A/*le.pdf", "/B/B_0")
        self.assertEqual(mock_write.call_count, 1)
        self.assertEqual(UUID_B0, self.repository.get_data()[UUID_FAIRYTALE]['parent'])
    ...
    def test_move_with_wild_card_single_collection_absolute_path(self, mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_ROOT)
        self.repository.process_move_command("/A/A_1", "/B")
        self.assertEqual(mock_write.call_count, 1)
        self.assertEqual(UUID_B, self.repository.get_data()[UUID_A1]['parent'])
    ...
    def test_move_with_wild_card_multiple_valid_documents(self, mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_A)
        self.repository.process_move_command("Fairytale*.pdf", "/B/B_0")
        self.assertEqual(mock_write.call_count, 2)
        self.assertEqual(UUID_B0, self.repository.get_data()[UUID_FAIRYTALE]['parent'])
        self.assertEqual(UUID_B0, self.repository.get_data()[UUID_FAIRYTALE_2]['parent'])

    @patch.object(RemarkableSSHMetadataSourceV2, "write_metadata")
    def test_move_with_wild_card_one_collection_exists_in_destination_one_collection_moved(self,
                                                                                           mock_write: MagicMock) -> None:
        mock_write.return_value = None
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.set_current_collection(UUID_B)
            self.repository.process_move_command("A*", "/A")
            self.assertEqual(mock_write.call_count, 1)
            self.assertEqual(UUID_A, self.repository.get_data()[UUID_A_UNDER_B]['parent'])
            output: str = mock_out.getvalue()
    ...
    def test_move_with_wild_card_one_document_has_invalid_metadata(self, mock_write: MagicMock) -> None:
        ...
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.set_current_collection(UUID_A)
            self.repository.process_move_command("*.pdf", "/B/B_0")
            self.assertEqual(mock_write.call_count, 2)
            self.assertEqual(UUID_B0, self.repository.get_data()[UUID_FAIRYTALE]['parent'])
            self.assertEqual(UUID_B0, self.repository.get_data()[UUID_FAIRYTALE_2]['parent'])
            output: str = mock_out.getvalue()
    ...
    @patch.object(RemarkableSSHMetadataSourceV2, "write_metadata")
    def test_move_with_wild_card_both_collections_and_documents_one_document_has_invalid_metadata_and_collection_with_same_name_exists_in_destination(
            self, mock_write: MagicMock) -> None:
        mock_write.return_value = None
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.set_current_collection(UUID_A)
            self.repository.process_move_command("*", "/B")
            ...
            self.assertTrue("mv: one or more metadata fields have invalid values: lastModified: -1" in output,
                            msg=f"Output was: {output}")
            self.assertTrue("mv: destination must not contain a child with the same name: Fairytale.pdf" in output,
                            msg=f"Output was: {output}")
            self.assertTrue("mv: destination must not contain a child with the same name: A_0" in output,
                            msg=f"Output was: {output}")

    @patch.object(RemarkableSSHMetadataSourceV2, "write_metadata")
    def test_move_with_wild_card_three_matching_collections_but_one_has_filename_already_present_in_destination(self,
                                                                                                                mock_write: MagicMock) -> None:
        mock_write.return_value = None
        self.repository.set_current_collection(UUID_B)
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_move_command("*", "/A")
            self.assertEqual(2, mock_write.call_count)
            self.assertEqual(UUID_A, self.repository.get_data()[UUID_B0]['parent'])
            self.assertEqual(UUID_A, self.repository.get_data()[UUID_A_UNDER_B]['parent'])
            output: str = mock_out.getvalue()
            self.assertTrue(f"mv: destination must not contain a child with the same name: A_0" in output,
                            msg=f"Output was: {output}")
            self.assertTrue(f"mv: destination must not contain a child with the same name: Fairytale.pdf" in output,
                            msg=f"Output was: {output}")
    ...
    def test_rename_positive_case(self, mock_write: MagicMock) -> None:
        ...
        self.repository.process_rename(target, new_visible_name)
        actual_visible_name = self.repository.get_visible_name_for_uuid(UUID_FAIRYTALE)
    ...
    def test_make_directory_with_valid_directory_name(self, mock_write: MagicMock) -> None:
        ...
        self.repository.process_mkdir(actual_path_to_make)

        args, kwargs = mock_write.call_args
        actual_path_uuid = args[0]

        self.assertEqual(mock_write.call_count, 1)
        self.assertIsNotNone(self.repository.get_data().get(actual_path_uuid),
                             msg=f"UUID {actual_path_uuid} not found in data")
        actual_dict = self.repository.get_data()[actual_path_uuid]
    ...
    def test_make_directory_with_directory_name_with_path_fails(self, mock_write: MagicMock) -> None:
        ...
        with patch('sys.stdout', new=StringIO()) as mock_out:
            ...
            output: str = mock_out.getvalue()
            self.assertTrue(
                "mkdir: /A/A_0: relative or absolute paths are not yet supported: hint: try help mkdir" in output,
                msg=f"Output was: {output}")
    ...
    def test_remove_one_document_positive_case(self, mock_remove: MagicMock) -> None:
        ...
        self.repository.set_current_collection(UUID_A)
        self.assertIn(UUID_FAIRYTALE, self.repository.get_data())
        self.repository.process_remove_command(target_pattern="Fairytale.pdf")
        self.assertEqual(mock_remove.call_count, 1)
        self.assertIsNone(self.repository.get_data().get(UUID_FAIRYTALE))
    ...
    def test_remove_two_documents_with_wildcard_positive_case(self, mock_remove: MagicMock) -> None:
        ...
        self.repository.set_current_collection(UUID_A)
        self.assertIn(UUID_FAIRYTALE, self.repository.get_data())
        self.assertIn(UUID_FAIRYTALE_2, self.repository.get_data())
        self.repository.process_remove_command(target_pattern="Fairytale*.pdf")
        self.assertEqual(mock_remove.call_count, 1)
        self.assertIsNone(self.repository.get_data().get(UUID_FAIRYTALE))
        self.assertIsNone(self.repository.get_data().get(UUID_FAIRYTALE_2))
    ...
    def test_remove_collection_recursively(self, mock_remove: MagicMock) -> None:
        ...
        self.assertTrue(expected_removals.issubset(self.repository.get_data()))

        self.repository.process_remove_command(target_pattern="A")
        self.assertEqual(mock_remove.call_count, 1)
    ...
    def test_remove_target_not_found(self, mock_remove: MagicMock) -> None:
        ...
        with patch('sys.stdout', new=StringIO()) as mock_out:
            self.repository.process_remove_command(target_pattern="no-such-target.pdf")
            output: str = mock_out.getvalue()
            self.assertTrue(
    ...
    def test_process_rcp_success_without_options(
        ...
        self.repository._traverse_path = MagicMock(return_value=UUID_ROOT)  # type: ignore[method-assign]

        source_file = "/path/to/file.pdf"
        target_path = "/"

        # ---- Execute ----
        self.repository.process_rcp_command_without_options(source_file, target_path)
        ...
        self.assertEqual(self.repository._data, ["new_data"])
    ...
    def test_process_rcp_success_with_valid_option_all(
        ...
        # ---- Execute ----
        self.repository.process_rcp_with_options(["-a", source_path, target_path])
    ...
    def test_process_rcp_success_with_valid_option_recurse(
        ...
        # ---- Execute ----
        self.repository.process_rcp_with_options(["-r", source_path, target_path])
    ...
    def test_process_rcp_with_valid_flags_source_not_found(
        ...
        # ---- Execute ----
        self.repository.process_rcp_with_options(["-a", source_file, target_path])
    ...
    def test_wild_card_match_finds_documents_with_pdf_extension(self) -> None:
        matches: List[str] = self.repository._get_matches_for_wildcard(UUID_A, "*.pdf")
    ...
    def test_wild_card_match_finds_document_matches_with_prefix(self) -> None:
        matches: List[str] = self.repository._get_matches_for_wildcard(UUID_A, "Fa*")
        self.assertEqual(2, len(matches))
        self.assertTrue(UUID_FAIRYTALE in matches)
        self.assertTrue(UUID_FAIRYTALE_2 in matches)

    def test_wild_card_match_finds_collection_matches_with_prefix(self) -> None:
        matches: List[str] = self.repository._get_matches_for_wildcard(UUID_A, "A_*")
        self.assertEqual(2, len(matches))
    ...
    def test_wild_card_alone_matches_all_children(self) -> None:
        matches: List[str] = self.repository._get_matches_for_wildcard(UUID_A, "*")
    ...
    def test_wild_card_with_multiple_stars_finds_matches(self) -> None:
        matches: List[str] = self.repository._get_matches_for_wildcard(UUID_A, "*valid*.pdf")
    ...
    def test_raises_not_found_exception_if_parent_is_not_a_parent(self) -> None:
        ...
        with self.assertRaises(NotFoundError) as ctx:
            self.repository._get_matches_for_wildcard(parent, entity_wildcard)
    ...
    def test_copy_file_from_host_to_target_when_remote_copy_fails(
        ...
        with patch("sys.stdout", new=StringIO()) as mock_out:
            self.repository._copy_file_from_host_to_target(source_file, target_uuid)
    ...
    def test_returns_true_when_entry_exists(self) -> None:
        self.assertTrue(
            self.repository._exists_visible_name_in_collection(UUID_FAIRYTALE, UUID_B)
        )

    def test_returns_false_when_entry_does_not_exist(self) -> None:
        self.assertFalse(
            self.repository._exists_visible_name_in_collection(UUID_FAIRYTALE, UUID_B0)
        )

    def test_raise_not_found_exception_if_entry_does_not_exist(self) -> None:
        with self.assertRaises(NotFoundError) as ctx:
            self.assertFalse(
                self.repository._exists_visible_name_in_collection("123-123", UUID_A)
            )
    ...
    def test_when_file_is_found_uuid_is_returned(self) -> None:
        actual_file_uuid: str = self.repository._get_uuid_with_visible_name_and_parent(
    ...
    def test_when_file_is_not_found_exception_is_raised(self) -> None:
        ...
        with self.assertRaises(NotFoundError) as context:
            self.repository._get_uuid_with_visible_name_and_parent(
                'Sadtale.pdf', UUID_B)
    ...
    def test_descendants_are_returned_correctly_for_path_root(self) -> None:
        actual_descendants: List[str] = self.repository._get_descendant_uuids(UUID_ROOT)
    ...
    def test_descendants_are_returned_correctly_for_path_a(self) -> None:
        actual_descendants: List[str] = self.repository._get_descendant_uuids(UUID_A)
        expected_descendants: List[str] = [
            UUID_A0, UUID_A1,
            UUID_FAIRYTALE, UUID_FAIRYTALE_2,
            UUID_INVALID_LAST_MODIFIED
    ...
    def test_when_no_descendants_exists_an_empty_list_is_returned(self) -> None:
        self.assertEqual([], self.repository._get_descendant_uuids(UUID_A0))
    ...
    def test_descendants_for_collection_b_are_returned_correctly(self) -> None:
        ...
        actual_uuids = self.repository._collect_uuids_matching_name_or_pattern_and_all_descendants_of_matches(
            "B*", UUID_ROOT
        )
    ...
    def test_visible_name_none_raises_invalid_metadata_error(self) -> None:
        ...
        self.repository._data[invalid_data_uuid] = invalid_data
        with self.assertRaises(InvalidMetadataError) as context:
            self.repository.get_visible_name_for_uuid(invalid_data_uuid)
    ...
    def test_parent_none_raises_invalid_metadata_error(self) -> None:
        ...
        self.repository._data[invalid_data_uuid] = invalid_data
        with self.assertRaises(InvalidMetadataError) as context:
            self.repository.get_parent(invalid_data_uuid)
    ...
    def test_when_collection_not_found_na_is_returned(self) -> None:
        invalid_data_uuid = "some-uuid"
        self.repository._data.pop(invalid_data_uuid, None)
    ...
    def test_when_parent_is_none_invalid_metadata_error_is_raised(self) -> None:
        ...
        self.repository._data[invalid_data_uuid] = invalid_data
        with self.assertRaises(InvalidMetadataError) as context:
            self.repository.generate_absolute_collection_path(invalid_data_uuid)
    ...
    def test_when_target_uuid_is_none_not_found_error_is_raised(
        ...
        with self.assertRaises(NotFoundError) as context:
            self.repository._validate_source_and_target_uuid("", "/foo", None)
    ...
    def test_generate_target_path_uuid_and_source_file_pairs_skips_empty_directory(
        ...
        with patch.object(
            ...
            result = self.repository._generate_target_path_uuid_and_source_file_pairs(
                source_path,
                files,
                target_collection,
            )
    ...
    def test_when_child_already_exists_it_is_returned(self) -> None:
        ...
        actual_uuid = self.repository._get_or_create_collection(parent_uuid, child_visible_name)
    ...
    def test_when_when_entity_is_not_a_collection_error_is_raised(self) -> None:
        with self.assertRaises(InvalidPathError) as context:
            self.repository._get_descendant_uuids(UUID_FAIRYTALE)
    ...
    def test_traverse_path_breaks_when_collection_pointer_is_not_string(self) -> None:
        ...
        with patch.object(
            ...
            result = self.repository._traverse_path("foo")
    ...
    def test_remove_entities_when_remove_fails(
        ...
        with patch("sys.stdout", new=StringIO()) as mock_out:
            self.repository._remove_entities(entity_uuids)
